...
/*   Updated: 2024/10/19 15:03:24 by dbaladro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sort(std::vector<int>& v, int depth) {
	...
	std::vector<int>::iterator	v_it, end = ((v.size() / step) % 2 ? v.begin() + (v.size() / step) * step - 1 : v.end() - 1);
	...
	std::vector<int>::iterator	res_it, to_insert_it; // vector iterator
	std::vector<int>::iterator	d_lim, h_lim, pos; // binary search variable

	if (to_insert.size() == 0)
		return ;

	// Init the main_chain and the pair_chain vector
...
void	sort(std::deque<int>& v, int depth) {
	...
	/* ----------   SWAP PART   ---------*/
	unsigned long	step = (1 << depth);	// define iterator step
	std::deque<int>::iterator	v_it, end = ((v.size() / step) % 2 ? v.begin() + (v.size() / step) * step - 1 : v.end() - 1);
	...
	int							diff;			// Jacobstahal diff
	int							res_len = step * (std::floor((v.size()/ (2.0 * step)) - 1) + 2);
	std::deque<int>			main(v.size()), to_insert(((v.size() - res_len) / step) * step);
	std::deque<int>::iterator	res_it, to_insert_it; // deque iterator
	std::deque<int>::iterator	d_lim, h_lim, pos; // binary search variable

	if (to_insert.size() == 0)
		return ;

	// Init the main_chain and the pair_chain deque
	std::copy(v.begin(), v.begin() + 2 * step, main.begin());
	v_it = v.begin() + 2 * step;
	res_it = main.begin() + 2 * step;
	to_insert_it = to_insert.begin();
